...
currencies = [
    ...
    {"currency": "BAM", "name": "Bosnia and Herzegovina convertible mark"},
    ...
    {"currency": "CLF", "name": "Unidad de Fomento"},
    ...
    {"currency": "MXV", "name": "Mexican Unidad de Inversion"},
    ...
    {"currency": "UYI", "name": "Uruguay Peso en Unidades Indexadas"},
...
async def country_details_by_name(country_name: str):
    sql_statement = sql()
    table = 'country'

    statement = sql_statement.select(table).where().id('country', country_name.capitalize()).sql_string
...
def character_search(string, text):
...
def exact_word_search(string, text):
    # convert all to lower cases
    string = string.lower()
    text = text.lower()
...
def country_code_by_name(country):
    for country_ in call_codes:
        if exact_word_search(country, country_['name']):
            return country_['number']


async def country_profile(country: str):
    sql_statement = sql()
    table = 'website_profile'
    statement = sql_statement.select(table).where().json_id(table, 'country', country).sql_string
